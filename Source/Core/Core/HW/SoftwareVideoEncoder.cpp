...
// standard BT.601 full-swing formulas, clamped since the exact math can
// spill a few units outside [0, 255] at the extremes.
...
// Scaled well down from Cemu's own port of this file (4000 kbps at
// 854x480): the GBA's native 240x160 (and every NegotiateVideo()-downscaled
// size below that) is a small fraction of that pixel area, so good-quality
// output needs proportionally less bitrate to stay well under typical home
// Wi-Fi throughput -- same reasoning, much smaller picture (azahar's own
// re-port of this file made the same adjustment for its fixed 320x240).
// Not resolution-adaptive beyond this one fixed choice -- a renegotiated
// session at a different size still gets the same budget. Both codecs
// share one value for now; H.265's better efficiency means it could go
// lower, but starting equal keeps this easy to reason about while tuning.
constexpr int kTargetBitrateKbps = 800;
...
constexpr int kVbvBufferKbits = 100;
      ...
      m_codedHeight(RoundUpTo16(height)), m_fps(fps == 0 ? 20 : fps)
{
  ...
  if (codec == VideoCodec::H264)
  {
    ...
    // threading (it inherently adds latency -- you need N frames in flight
    // for N threads), so x264 falls back to auto-detecting the host's CPU
    // count and using real SLICE-based threading instead -- splitting one
    // picture into multiple separate slice NALs, one per thread. Confirmed
    // live: New3DS's MVD hardware decoder accepts the first two slices
    // fine (MVD_STATUS_INCOMPLETEPROCESSING) but returns an undocumented
    // status code on the third and crashes the whole mvd sysmodule
    // processing a fourth, rather than correctly handling a multi-slice
    // picture -- every client this project ships assumes one NAL per
    // picture (docs/protocol.md's Annex-B framing was never designed
    // around multi-slice pictures). Losing x264's own slice-parallel
    // speedup costs some encode-side headroom, but every client gets the
    // simpler, single-slice-per-picture stream this produces, not just
    // the 3DS.
    param.i_threads = 1;
    ...
    // Repeats SPS/PPS before every keyframe (not just the first) -- lets a
    // decoder that missed the session's very first frame (or desynced)
    // still recover cleanly from any later forced keyframe, not only
    // session start.
    ...
    // spec-compliant decoder was stretching an already-full-range signal
    // as if it still had headroom to expand -- reported live as an
    // "oversaturated"/too-contrasty image on iOS's VideoToolbox decoder
    // (CompressedVideoDecoder.swift), which -- unlike some software
    // decoders that happen to guess permissively -- actually follows the
    // spec's default here. i_colorprim/i_transfer/i_colmatrix = 6
    // (SMPTE170M, i.e. ITU-R BT.601) matches ConvertRgba8ToI420()'s own
    // coefficients (0.299/0.587/0.114) exactly, rather than leaving those
    // at x264's own default (2 = unspecified), so every decoder now gets
    // the same explicit, correct answer instead of guessing at all three
    // independently.
    param.vui.b_fullrange = 1;
    param.vui.i_colorprim = 6;
    param.vui.i_transfer = 6;
    param.vui.i_colmatrix = 6;
    // Capped bitrate (ABR + a VBV ceiling at the same rate), not CRF: CRF
    // targets a *quality* level with no bound on the resulting bitrate, so
    // a busy/fast-changing scene can spike well past what a real Wi-Fi link
    // sustains in real time -- see Cemu's own port of this file for the
    // concrete decode-side backlog this was found from.
    ...
    // Baseline (CAVLC entropy coding), not Main (CABAC): see Cemu's own
    // port of this file -- a real hardware decoder showed persistent
    // tearing/distortion with CABAC that Baseline's simpler entropy coding
    // avoids, at some compression-efficiency cost the bitrate cap above
    // already leaves headroom for.
  ...
  else
  {
    ...
    // Same full-range/BT.601 VUI signaling as the H.264 branch above, same
    // reasoning (its own comment) -- ConvertRgba8ToI420()'s full-range
    // output was never being declared as such.
...
void SoftwareVideoEncoder::ConvertRgba8ToI420(const uint8_t* rgba8)
{
  ...
  // 4:2:0 chroma: one U/V sample per 2x2 luma block, averaged from the 4
  // source pixels it covers rather than just point-sampling one of them,
  // for a cleaner downscale. Same edge-replication as above for any 2x2
  // block that falls partly or fully in the padding region.
...
bool SoftwareVideoEncoder::EncodeFrame(const uint8_t* rgba8, std::vector<uint8_t>& outNals)
{
  ...
  {
    // Not x265_picture_init(): that takes the x265_param* the encoder was
    // opened with (to derive bitDepth/colorSpace defaults), which isn't
    // kept around past x265_encoder_open() above -- passing nullptr
    // crashes inside x265_picture_init itself. A plain zero-init is
    // equivalent here since every field x265_picture_init would otherwise
    // have derived from param (colorSpace, bitDepth) is set explicitly
    // below anyway.
    x265_picture picIn;
